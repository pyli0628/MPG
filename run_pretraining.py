...
from loader import DataLoaderMasking #, DataListLoader
from model import Pretrain_model,Pretrain_MolGNet
...
def create_pretraining_dataset(input_file,  args, worker_init):
    train_data = PretrainDataset("../dataset/" + args.dataset, rank = input_file)
...
class PretrainingCriterion(torch.nn.Module):
    def __init__(self):
        super(PretrainingCriterion, self).__init__()
        self.loss_fn = torch.nn.CrossEntropyLoss()
    def forward(self, pred_node,pred_graph,mask_node_label,ngp_y):
        masked_lm_loss = self.loss_fn(pred_node.double(), mask_node_label)
        cgp_loss = self.loss_fn(pred_graph.double(), ngp_y)
        total_loss = masked_lm_loss + cgp_loss
        return total_loss
    ...
    if args.do_train:
        ...
        while True:
            ...
            if restored_data_loader is None:
                train_data = PretrainDataset("../dataset/" + args.dataset,rank=data_file)
                train_sampler = RandomSampler(train_data)
                train_dataloader = DataLoaderMasking(train_data, sampler=train_sampler,
                                              batch_size=args.train_batch_size * args.n_gpu,
                                              num_workers=4, worker_init_fn=worker_init,
...
if __name__ == "__main__":
    ...
    now = time.time()
    args, final_loss, final_maskacc, final_cgpacc, train_time_raw, global_step = main()
    ...
    if is_main_process():
        ...
        dllogger.log(step=tuple(), data={"e2e_train_time": e2e_time, "training_sequences_per_second": training_perf,
                                         "final_loss": final_loss, "raw_train_time": train_time_raw,"final_maskacc":final_maskacc,
                                                                            "final_cgpacc":final_cgpacc })
    dllogger.flush()
